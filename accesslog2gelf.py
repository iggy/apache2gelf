...
As specified above, this requires the following line in apache configuration:
    ...
    CustomLog "||/path/to/accesslog2gelf.py" "- %V %h %{X-Forwarded-For}i %u \"%r\" %>s %b \"%{Referer}i\" \"%{User-agent}i\""

"""
regexp = '^(?P<virtualhost>\S+) (?P<host>\S+) (?P<ipaddr>\S+) (?P<xfwdfor>\S+) (?P<username>\S+) "(?P<request>[^"]*)" (?P<status>\S+) (?P<size>\S+) "(?P<referer>[^"]*)" "(?P<useragent>[^"]*)"$'
...
for line in iter(sys.stdin.readline, b''):
    ...
    if matches:
        ...
        if args.vhost:
            adapter.info('%s %s %s %s %s %s "%s" %s %s "%s" "%s"' % tuple(record[f] for f in ["virtualhost", "ipaddr", "vhost", "host", "xfwdfor", "username", "request", "status", "size", "referer", "useragent"]))
        else:
            adapter.info('%s %s %s %s %s "%s" %s %s "%s" "%s"' % tuple(record[f] for f in ["virtualhost", "ipaddr", "host", "xfwdfor", "username", "request", "status", "size", "referer", "useragent"]))
